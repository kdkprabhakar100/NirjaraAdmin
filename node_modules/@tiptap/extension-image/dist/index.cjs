...
*/
const Image = _tiptap_core.Node.create({
	name: "image",
	addOptions() {
		return {
			inline: false,
	...
	addNodeView() {
		...
		return ({ node, getPos, HTMLAttributes, editor }) => {
			...
			const onUpdate = (updatedNode) => {
				...
				const newHTMLAttributes = (0, _tiptap_core.getRenderedAttributes)(updatedNode, editor.extensionManager.attributes.filter((attribute) => attribute.type === updatedNode.type.name));
			...
			const nodeView = new _tiptap_core.ResizableNodeView({
				element: el,
				editor,
				node,
				getPos,
